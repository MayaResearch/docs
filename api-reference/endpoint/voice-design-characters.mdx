...
// Filter by gender (extract from name)
const femaleVoices = voices.filter(v => v.name.includes('female'));

// Filter by accent
const americanVoices = voices.filter(v => v.name.includes('American'));

// Filter by age
const twentiesVoices = voices.filter(v => v.name.includes('20s'));

// Get voice by ID
const avaVoice = voices.find(v => v.voice_id === 'Ava');
```
